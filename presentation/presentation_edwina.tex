...
\begin{frame}{Brauer relations}
    Let $\{ X_i \colon i \in I\}$, $\{X_j \colon i \in J \}$ be transitive finite $G$-sets. If $G$ is non-cyclic, it can happen that
...
\begin{frame}{Brauer relations}
    This corresponds to  
    \[ \bigoplus_{i \in I} \Ind_{H_i}^G \trivial \ominus \big{(}\bigoplus_{j \in J} \Ind_{H_j}^G \trivial\big{)} = 0 \] 
...
\begin{frame}{Brauer relations : Applications}
    \color{red}{Examples of $L$ functions, Dedekind zeta functions ... Brauer relations provide interesting relations between some number theoretic objects}
\end{frame}
...
\begin{frame}{Counting primes}
    ...
    \vspace{1em}
    For $H \leq G$, the primes of $L = F^H$ above $\fp$ are in one-to-one correspondence with the double cosets $H  \backslash G / D$. \pause
...
\begin{frame}{D-local functions}
    {\color{red} something to say here.}
\end{frame}
...
\begin{frame}
    ...
    c_{\fP}(E / K) \cdot d_{\fP}(E / K, \omega) = \prod_p C_{\fP \mid p}(E / K)
    $$
...
\begin{frame}{Residue degree one}
    ...
    \begin{lemma}
       ...
       \[ C_{\fP \mid \fp}(E / K') \equiv c_{\fp}(E / K)\cdot d_{\fp}(E / K, \omega) \mod (\bQ)^{\times 2} \]
    \end{lemma}\pause
...
\begin{frame}{Only certain $D_p$}
    \begin{lemma}
        ...
        Consider $p$ with decomposition group $D_p$ of exponent $b$. \pause 
...
\begin{frame}{Outlook : Other future work}
    \begin{itemize}
        ...
        \item There should be an analogous norm relations test for hyperelliptic curves and abelian varieties. Perhaps we could find more interesting examples here.
    \end{itemize}
\end{frame}
\end{section} 
