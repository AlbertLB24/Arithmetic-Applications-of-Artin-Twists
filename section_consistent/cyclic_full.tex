...
\begin{lemma}\label{lem_cyclic_decomp}
    ...
    \begin{equation}\label{eqn_cyclic_relation}
        \mathcal{N}_{\QQ(\zeta_d)/\QQ}(\chi)=\sum_{\mathfrak{g}\in\Gal(\QQ(\zeta_d)/\QQ)}\chi^{\mathfrak{g}}=\sum_{k\mid d}\mu(d/k)\Ind_{L_k/\QQ}\mathds{1}.
    \end{equation}
...
We divide the proof of Theorem \ref*{thm_consistent_cyclic} into two cases: odd and even cyclic extensions. The main idea in both cases is to simplify the general case into smaller cases where we can directly compute $\contr_\chi(\pp)$ for each finite place $\pp$ of $K$. During the proof, we will also need the following result about quadratic subfields of cyclotomic extensions.

\begin{lemma}
    Let $p$ be a rational prime, $n$ a positive integer and let $p^*=(-1)^{(p-1)/2}p$. Then the quadratic subfields of $\QQ(\zeta_{p^n})$ are
    \begin{itemize}
        \item $\QQ(\sqrt{p^*})$ if $p$ is odd,
        \item $\QQ(i)$ if $p=n=2$,
        \item $\QQ(i),\QQ(\sqrt{2})$ and $\QQ(\sqrt{-2})$ if $p=2$ and $n\geq 3$.
    \end{itemize}
\end{lemma}

\begin{proof}
    If $p$ is odd, then $\Gal(\QQ(\zeta_{p^n})/\QQ)=(\ZZ/p^n\ZZ)^*=C_{p^{n-1}(p-1)}$ is a cyclic group of even order, and therefore $\QQ(\zeta_{p^n})$ has one unique quadratic subfield. Hence, it suffices to find the unique quadratic subfield of $\QQ(\zeta_p)$.
    Two proofs:
    \begin{enumerate}
        \item A simple calculation shows that 
        $$\left(\sum_{a=0}^{p-1}\left(\frac{a}{p}\right)\zeta_p^a\right)^2=(-1)^{(p-1)/2}p,$$
        and therefore $\sqrt{p^*}\in\QQ(\zeta_p)$ as desired.
        \item Let $\QQ(\sqrt{D})\subseteq\QQ(\zeta_{p^n})$ be a quadratic subfield. The extension $\QQ(\zeta_{p^n})/\QQ$ only ramifies at $p$ and therefore $\QQ(\sqrt{D})$ can only (in fact, it must) ramify at $p$. If $p\equiv 1\pmod{4}$, then the only such field is $\QQ(\sqrt{p})$ and if $p\equiv 3\pmod{4}$, then the field is $\QQ(\sqrt{-p})$.
    \end{enumerate} 
    If $p=n=2$, then $\QQ(\zeta_4)=\QQ(i)$ so there is nothing to prove. If $p=2$ and $n\geq 3$, then $\Gal(\QQ(\zeta_{2^n})/\QQ)=(\ZZ/2^n\ZZ)^*=C_2\times C_2^{n-2}$ and therefore $\QQ(\zeta_{2^n})$ has three quadratic subfields, and therefore it is enough to find the quadratic subfields of $\QQ(\zeta_8)$. Since $\zeta_8=(1+i)/\sqrt{2}$, it is clear that the quadratic subfields are $\QQ(i)$, $\QQ(\sqrt{2})$ and $\QQ(\sqrt{-2})$ as desired.
...
\begin{proof}
    ...
    Now assume $\pp$ has additive reduction, and let $p\ZZ=\pp\cap\QQ$. By assumption, $p\neq2,3$. Firstly, we note that $D_{\mathfrak{P}\mid\pp}(F/K)=1$ unless $\pp$ ramifies in $F/K$, and in that case it is a power of $N_{K/\QQ}(\pp)=p^s$. If $s$ is even, we are immediately done, so assume instead that $s$ is odd. If $L_\mathfrak{P}/K_\pp$ is wildly ramified, then $p=q$ is a norm from $\QQ(\sqrt{q^*})$. If $L_\mathfrak{P}/K_\pp$ is tamely ramified, then by Lemma \textbf{Add here reference of Lemma for tame totally ramified extensions}, it follows that $q\mid p^r-1$ and therefore 
    \begin{equation}
    ...
    Finally, we need to consider the Tamagawa numbers. Note that since $q$ is odd, if $\mathfrak{P}$ is any prime in $F$ above $\pp$, then $\sqrt{D}\in K_\pp$ if and only if $\sqrt{D}\in L_\mathfrak{P}$ for any $D\in\QQ$. Moreover, if $q\neq 3$, then $\gcd(e_{\mathfrak{P}\mid\pp},12)=1$ and by Lemma \ref{lem_add_tam}, $c_\pp(E/K)=c_\mathfrak{P}(E/F)$. Therefore, we get a square in this case.
    If $q=3$, we have $\gcd(e_{\mathfrak{P}\mid\pp},12)=1$ unless $L_\mathfrak{P}/K_\pp$ is a ramified extension. 
