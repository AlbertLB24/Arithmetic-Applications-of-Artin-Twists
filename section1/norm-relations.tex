...
\begin{example}
    Let $G = C_p$ for $p$ a prime. Let $\rho$ be a character of degree $p$, so $\bQ(\rho) = \bQ(\zeta_p)$. There is a unique $\rho$-relation given by $\Theta = C_1 - C_p$. Let $\psi(H) = [G \colon H]$. Then $\psi(\Theta)$ is a norm relation. We have $\psi(\Theta) = p \in \fieldnorm{\rho}$ since the element $1 - \zeta_p$ has norm $p$. {\color{red} check this is true lol}
\end{example}
